...
from ultralytics.nn.modules import (
    ...
    C2f,
    StarC2f,  # StarC2f
    C2f_BiLevelRoutingAttention,  # BiC2f
    C2fAttn,
    C2fCIB,
    C2fPSA,
    C3Ghost,
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    # 从yaml文件中读取每个层
    # f n m args实际上对应每一个[-1, 1, Conv, [64, 3, 2]]这样的参数
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        m = (
            # 效果是将str类型的m转换为实际的Module
            # 如果yaml文件中使用的已经是torch.nn中的模块了，则不需要torch.nn作为头部
        ...
        if m in base_modules:
            ...
            args = [c1, c2, *args[1:]] # in out *args[1:] 也即 in out kernel_size stride
            if m in repeat_modules:
                args.insert(2, n)  # number of repeats [c1, c2, n, True]对应c2f的参数
                n = 1
        ...
        layers.append(m_)  # 将实例化好的模块添加到层中
        if i == 0:
            ch = []
        ch.append(c2)  # 将本模块的out channel添加到ch中，作为下一个模块的input channel
