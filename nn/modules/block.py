...
class StarNetBlock(nn.Module):
    def __init__(self, dim, mlp_ratio=3, drop_path=0.):
        ...
        self.act = nn.ReLU6()
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()  # DropPath>0时进行正则化否则使用Identity即不进行任何操作
...
class StarSPPF(nn.Module):
    ...
    def __init__(self, c1, c2, k=5):
        ...
        c_ = c1 // 2  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.starBlock1 = StarNetBlock(c_)
        self.cv2 = Conv(c_ * 4, c2, 1, 1)
        self.starBlock2 = StarNetBlock(c2)
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
    ...
    def forward(self, x):
        ...
        y = [self.starBlock1(x)]
        y.extend(self.m(y[-1]) for _ in range(3))
        return self.starBlock2(self.cv2(torch.cat(y, 1)))
...
class StarC2f(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.fc = ConvBN(self.c, self.c, kernel_size=1, stride=1, with_bn=False)
        self.m = nn.ModuleList(Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))

    def forward(self, x):
        y = self.cv1(x)
        y = list(y.chunk(2, dim=1))
        star_y = self.fc(y[0]) * self.fc(y[1])
        y[1] = star_y
        y.extend(m(y[-1]) for m in self.m)
